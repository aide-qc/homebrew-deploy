...
def run_command(cwd, command, *args):
    commands = [command] + list(args)
    try:
        result = subprocess.run(commands, cwd=cwd, check=True, stdout=subprocess.PIPE)
...
def compute_sha256sum(filename):
    result = run_command(os.getcwd(), 'sha256sum', filename)
...
run_command(os.getcwd()+'/qcor', 'git', 'pull')
xacc_commit_hash = run_command(os.getcwd()+'/xacc', 'git', 'rev-parse', 'HEAD')[0].decode('utf-8')
qcor_commit_hash = run_command(os.getcwd()+'/qcor', 'git', 'rev-parse', 'HEAD')[0].decode('utf-8')
git_message = ''

print('xacc commit hash is {}'.format(xacc_commit_hash))
print('qcor commit hash is {}'.format(qcor_commit_hash))

homebrew_changes_occured = False
if xacc_commit_hash != cached_sums['xacc']['git']:
